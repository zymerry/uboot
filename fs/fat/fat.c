...
int fat_register_device (block_dev_desc_t * dev_desc, int part_no)
{
	...
	if (dev_desc->block_read(dev_desc->dev, 0, 1, (ulong *)buffer) != 1) {
		debug("** Can't read from device %d **\n",
			dev_desc->dev);
		return -1;
	}
	if (buffer[DOS_PART_MAGIC_OFFSET] != 0x55 ||
		...
		/* no signature found */
		debug ("** FAT: not siganature found **\n");
		return -1;
	}
...
/*****    txmcu   *****/
static int disk_read_tx (__u32 startblock, __u32 getsize, __u8 * bufptr, int flag)
{
	if (cur_dev == NULL)
		return -1;
	...
	if (cur_dev->block_read_tx) {
		return cur_dev->block_read_tx(cur_dev->dev, startblock, getsize,
					   (unsigned long *) bufptr, flag);
	}
	return -1;
}

static int get_cluster_tx(fsdata *mydata, __u32 clustnum, __u8 *buffer, unsigned long size, int flag)
{
	int idx = 0;
	__u32 startsect;
	...
	if (disk_read_tx(startsect, size / FS_BLOCK_SIZE, buffer, flag) < 0) {
	...
	if (size % FS_BLOCK_SIZE) {
		...
		if (disk_read_tx(startsect + idx, 1, tmpbuf, flag) < 0) {
			debug("Error reading data\n");
			return -1;
		}
	      ...
	      unsigned long maxsize)
{
	...
	do {
		...
		if (get_cluster_tx(mydata, curclust, buffer, (int)actsize, usbflag) != 0) {
		...
		buffer += actsize;
		actsize = filesize;
		if (get_cluster_tx(mydata, endclust, buffer, (int)actsize, usbflag) != 0) {
			debug("Error reading cluster\n");
			return -1;
		}
		gotsize += actsize;
...
int file_fat_detectfs (void)
{
	...
	debug("Partition %d: Filesystem: %s \"%s\"\n", cur_part,
...
long file_fat_read (const char *filename, void *buffer, unsigned long maxsize)
{
	...
	if(! strcmp(filename,"u-boot.bin")) {
		...
		for(i=0;i<10;i++){
			#if ( LCD_TFT == TFT480272 )
			tx2440_Draw_ASCII(j,125,0xe010,arr_uboot[i]);
			#elif ( LCD_TFT == TFT800480 )
			tx2440_Draw_ASCII(j+160,200,0xe010,arr_uboot[i]);
			#endif
			j += 10;
		}
	}

	if(! strcmp(filename,"zImage")) {
		#if ( LCD_TFT == TFT480272 )
		tx2440_lcd_ClearScr_Rectangle( 190,125,200,20, BACK_COLOR);
		#elif ( LCD_TFT == TFT800480 )
		tx2440_lcd_ClearScr_Rectangle( 350,200,200,20, BACK_COLOR);
		...
		for(i=0;i<6;i++){
			#if ( LCD_TFT == TFT480272 )
			tx2440_Draw_ASCII(j,125,0xe010,arr_zImage[i]);
			#elif ( LCD_TFT == TFT800480 )
			tx2440_Draw_ASCII(j+160,200,0xe010,arr_zImage[i]);
			#endif
			j += 10;
		}
	}

	if(! strcmp(filename,"system.img")) {
		#if ( LCD_TFT == TFT480272 )
		tx2440_lcd_ClearScr_Rectangle( 190,125,200,20, BACK_COLOR);
		#elif ( LCD_TFT == TFT800480 )
		tx2440_lcd_ClearScr_Rectangle( 350,200,200,20, BACK_COLOR);
		...
		for(i=0;i<10;i++){
			#if ( LCD_TFT == TFT480272 )
			tx2440_Draw_ASCII(j,125,0xe010,arr_system[i]);
			#elif ( LCD_TFT == TFT800480 )
			tx2440_Draw_ASCII(j+160,200,0xe010,arr_system[i]);
			#endif
			j += 10;
		}
	}
