...
unsigned long usb_stor_read_tx(int device, unsigned long blknr, unsigned long blkcnt, unsigned long *buffer, int flag);
...
int usb_stor_scan(int mode)
{
	...
	for(i=0;i<USB_MAX_STOR_DEV;i++) {
		...
		usb_dev_desc[i].block_read_tx=usb_stor_read_tx;
	}
...
unsigned long usb_stor_read(int device, unsigned long blknr, unsigned long blkcnt, unsigned long *buffer)
{
	unsigned long start,blks,myblks, buf_addr;
...
retry_it:
		if(smallblks==USB_MAX_READ_BLK) {
			//usb_show_progress();
			printf("\r txmcu - usb_read: -- %d\%. ",((myblks-blks)*100)/myblks);
		}
...
#include "../drivers/lcd/tx2440_lcd_config.h"
...
retry_it:
		if(smallblks==USB_MAX_READ_BLK) {
			//usb_show_progress();
			if(!flag){
				printf("\rReading data: %d\%",((myblks-blks)*100)/myblks);
				#if ( LCD_TFT == TFT480272 )
				tx2440_percent((myblks-blks)*100/myblks,200,160);
				tx2440_drawProcessBar(myblks, myblks-blks, 10, LCD_YSIZE_TFT - 90);
				#elif ( LCD_TFT == TFT800480 )
				tx2440_percent((myblks-blks)*100/myblks,350,230);
				tx2440_drawProcessBar(myblks, myblks-blks, 10, 260);
				#endif
				tx2440_lcd_EnvidOnOff(1);
			}
			else {
				if(1==sii) {
					printf("\rReading data: %d\%",((myblks-blks)*50)/myblks);
					#if ( LCD_TFT == TFT480272 )
					tx2440_percent(((myblks-blks)*50)/myblks, 200, 160);
					tx2440_drawProcessBar(2*myblks, myblks-blks, 10, LCD_YSIZE_TFT - 90);
					#elif ( LCD_TFT == TFT800480 )
					tx2440_percent(((myblks-blks)*50)/myblks, 350, 230);
					tx2440_drawProcessBar(2*myblks, myblks-blks, 10, 260);
					#endif
					tx2440_lcd_EnvidOnOff(1);
				}
				if (2==sii) {
					printf("\rReading data: %d\%",((myblks-blks)*50)/myblks+50);
					#if ( LCD_TFT == TFT480272 )
					tx2440_percent(((myblks-blks)*50)/myblks+50, 200, 160);
					tx2440_drawProcessBar(2*myblks, 2*myblks-blks, 10, LCD_YSIZE_TFT - 90);
					#elif ( LCD_TFT == TFT800480 )
					tx2440_percent(((myblks-blks)*50)/myblks+50, 350, 230);
					tx2440_drawProcessBar(2*myblks, 2*myblks-blks, 10, 260);
					#endif
					tx2440_lcd_EnvidOnOff(1);
				}
			}
	...
	if(1==sii2) {
		...
		tx2440_lcd_ClearScr_Rectangle(0,LCD_YSIZE_TFT - 90,LCD_XSIZE_TFT,10, 0xF800);
		#elif ( LCD_TFT == TFT800480 )
		tx2440_percent(100, 350, 230);
		tx2440_lcd_ClearScr_Rectangle(0,260,LCD_XSIZE_TFT,10, 0xF800);
		#endif
	}
