...
int nand_write_opts(nand_info_t *meminfo, const nand_write_options_t *opts)
{
    ...
    while (imglen && (mtdoffset < meminfo->size)) {
        ...
        /* skip the first good block when wirte yaffs image, by www.txmcu.com */
        if (skipfirstblk) {
        ...
        if (!opts->quiet) {
            ...
            if (percent != percent_complete) {
				...
				tx2440_percent(percent, 200, 220);
				tx2440_drawProcessBar(100, percent, 10, LCD_YSIZE_TFT - 20);
				#elif ( LCD_TFT == TFT800480 )
				tx2440_percent(percent, 350, 300);
				tx2440_drawProcessBar(100, percent, 10, 330);
				#endif
                printf("\rWriting data at 0x%x ""-- %3d%% complete.", mtdoffset, percent);
                percent_complete = percent;
            }
        }
